...
import { getMMMContributionChart, MMMResponseCurvesVegaChart } from "@/lib/api/mmm"

interface MeridianContributionChartProps {
...
function getCacheKey(timeGranularity: string): string {
  return `contrib_${timeGranularity}`
}
...
export function MeridianContributionChart({
  timeGranularity = "quarterly",
}: MeridianContributionChartProps) {
  ...
  useEffect(() => {
    let cancelled = false
    const cacheKey = getCacheKey(timeGranularity)
    const now = Date.now()
    ...
    if (!entry || isExpired) {
      ...
      const requestPromise = getMMMContributionChart({
        timeGranularity,
      })
  ...
  useEffect(() => {
    ...
    async function renderChart() {
      ...
      try {
        ...
        console.error("Failed to render contribution chart", err)
      }
    }
    ...
    return (
      <div className="flex items-center justify-center p-12">
        <div className="text-center">
          <div className="mb-2 text-sm text-muted-foreground">Loading contribution chart...</div>
